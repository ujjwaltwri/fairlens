...
Each report contains:
  ...
  - Gemini AI narrative + recommendation (if provided)
  - Severity ratings and recommended actions

Signature (backward-compatible):
  generate_reports(dataset_name, data_audit, model_audit, mitigation_results,
                   gemini_narrative=None, gemini_recommendation=None)
"""
...
def build_json_report(
    ...
    report = {
        "report_metadata": {
            ...
            "report_version": "2.0",
            "gemini_enhanced": gemini_narrative is not None,
        },
        "data_audit": {
...
def _get_recommendations(data_audit, model_audit):
    ...
    for attr, r in data_audit.get("attribute_results", {}).items():
        ...
        if r.get("chi2_significant"):
            recs.append(
                f"Protected attribute '{attr}' is statistically associated with outcome "
                f"— investigate proxy features"
            )

    if tpr_gap > 0.1:
        recs.append(
            f"True positive rate gap of {tpr_gap:.3f} detected "
            f"— apply equalized odds post-processing"
        )
    if flip_rate > 0.1:
        recs.append(
            f"Counterfactual flip rate {flip_rate:.1%} "
            f"— model is directly using protected attribute"
        )
    if score >= 70:
...
def generate_pdf_report(report_data, dataset_name):
    ...
    justify_style = ParagraphStyle("Justify", parent=styles["Normal"],
                                    fontSize=10, spaceAfter=6, leading=15,
                                    alignment=TA_JUSTIFY)
    small_style = ParagraphStyle("Small", parent=styles["Normal"],
                                  fontSize=8.5, textColor=colors.HexColor("#666666"))
    gemini_box_style = ParagraphStyle("GeminiBox", parent=styles["Normal"],
                                       fontSize=10, spaceAfter=6, leading=15,
                                       alignment=TA_JUSTIFY,
    ...
    gem  = report_data.get("gemini_analysis", {})
    ...
    story.append(Paragraph(
        ...
        + ("  |  Gemini-enhanced ✓" if meta.get("gemini_enhanced") else ""),
        small_style
    ))
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor("#DDDDDD")))
    story.append(Spacer(1, 0.3*cm))

    # ── Gemini Narrative (if available) ──
    narrative = gem.get("narrative")
    if narrative:
        story.append(Paragraph("AI-generated analysis", h2_style))
        # Tinted background box via a 1-cell Table
        narrative_table = Table(
            [[Paragraph(narrative, gemini_box_style)]],
            colWidths=[16.5*cm]
        )
        narrative_table.setStyle(TableStyle([
            ...
            ("ROUNDEDCORNERS", [6]),
            ("PADDING",     (0,0), (-1,-1), 10),
            ("BOX",         (0,0), (-1,-1), 0.5, colors.HexColor("#534AB7")),
        ]))
    ...
    if recommendation:
        story.append(Spacer(1, 0.3*cm))
        story.append(Paragraph("AI deployment recommendation", h2_style))
        rec_table = Table(
            [[Paragraph(recommendation, gemini_box_style)]],
            colWidths=[16.5*cm]
        )
    ...
    story.append(Paragraph(
        "This report was generated by the FairLens Bias Audit System. "
...
def generate_reports(
    ...
    Args:
        dataset_name          : short name used for filenames (e.g. 'adult')
        data_audit            : dict from data_auditor.audit_dataset()
        model_audit           : dict from model_auditor.audit_model()
        mitigation_results    : dict from mitigation.apply_all_mitigations()
        gemini_narrative      : optional plain-text narrative from gemini.generate_narrative()
        gemini_recommendation : optional plain-text recommendation from gemini.generate_recommendation()

    Returns:
        dict with keys: 'json', 'pdf', 'report_data'
    """
...
def generate_all_reports(datasets_dict, data_audits, model_audits, mitigations,
                          gemini_narratives=None, gemini_recommendations=None):
    """Generate reports for all datasets."""
    gemini_narratives     = gemini_narratives or {}
    gemini_recommendations = gemini_recommendations or {}
    all_paths = {}
